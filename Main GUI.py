...
def plotScatterMatrix(df1, plotSize, textSize):
    ...
    df1 = df1[columnNames]
    ax = pd.plotting.scatter_matrix(df1, alpha=0.75, figsize=[
...
def scatterplt(disea):
    # total sum of symptom reported for given disease
    x = ((df.loc[disea]).sum())
...
def DecisionTree():
    if len(NameEn.get()) == 0:
        ...
        sym = messagebox.askokcancel(
            "System", "Kindly Fill atleast first two Symptoms")
        if sym:
            root.mainloop()
    else:
        print(NameEn.get())
        ...
        scatterinp(Symptom1.get(), Symptom2.get(),
...
knnLb = Label(root, text="kNearestNeighbour",
              fg="Red", bg="Sky Blue", width=20)
...
rs = Button(root, text="Reset Inputs", command=Reset,
            bg="yellow", fg="purple", width=15)
rs.config(font=("Times", 15, "bold italic"))
rs.grid(row=10, column=3, padx=10)

ex = Button(root, text="Exit System", command=Exit,
            bg="yellow", fg="purple", width=15)
